...
- ✅ **执行验证**: 逐步执行任务并验证结果
- 📚 **RAG 支持**: 基于文档的检索增强生成
- 💬 **交互界面**: 友好的 Web 界面（Google 风格设计 + 炫酷动画）
- 🚀 **实时反馈**: WebSocket 实时推送执行状态
- 📁 **文件上传**: 支持文件和文件夹上传
- 💭 **思考过程**: 实时显示模型思考过程
...
- **UI 层**: 
  ...
  - 原生 Fetch API
  - 自定义 Markdown 渲染
  - **动画库**: particles.js（粒子背景）、anime.js（高级动画）
  - **零依赖**: 不使用 Vue/React/npm，纯原生实现
...
功能：
- 🎨 **Google 风格界面**: 简洁现代的设计
- ✨ **炫酷动画效果**: 粒子背景、Logo 交互动画、按钮特效
- 📝 **输入需求**: 支持多行文本输入任何问题
- 📁 **文件上传**: 支持选择文件或文件夹上传
- 🚀 **执行任务**: 点击"执行"按钮开始处理
- 📊 **实时日志**: WebSocket 实时显示执行过程
- 💭 **思考过程**: 显示模型思考步骤（紫色主题面板）
- 📋 **查看结果**: Markdown 格式展示最终结果
- 🔗 **连接状态**: 右下角显示 WebSocket 连接状态

详细说明请参考：[UI 使用指南](UI 使用指南.md)
...
**说明**: 建立 WebSocket 连接，用于实时接收 Agent 执行状态

**消息类型**:
- `connected`: 连接成功
- `thought`: 模型思考过程（💭 显示在紫色面板）
- `log`: 日志信息
- `progress`: 进度更新
- `result`: 最终结果
- `error`: 错误信息
